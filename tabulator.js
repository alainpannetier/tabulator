 ...
 */
 ...
 (function(){
  ...
  mouseDragWidth:false, //starting width of column on mouse drag
  ...
  filterCustom:{}, // Chain of custom filters

  paginationCurrentPage:1, // pagination page
  paginationMaxPage:1, // pagination maxpage

  progressiveRenderTimer:null, //timer for progressive rendering
  progressiveRenderFill:false, //initial fill of a progressive rendering table

  columnList:[], //an array of all actual columns ignoring column groupings

  responsiveColumnList:[], //array of responsive columns
  responsiveColumnIndex:0, //current position in responsive array
  ...
  options: {
    ...
    progressiveRenderMargin:200, //distance in px before end of scroll before progressive render is triggered
    ...
    columns:false,//store for column header info
    ...
    sortBy:"id", //default column to sort by
    ...
    selectableCheck:function(data, row){return true;}, //check whether row is selectable
  ...
  _parseTable:function(){
    ...
    //build columns from table header if they haven't been set;
    if(headers.length){
  ...
  _buildElement: function(){
    ...
    //// backwards compatibility options adjustments ////

    //old persistent column layout adjustment
  ...
  //return the language definitions for the current locale
  getLang:function(){
    var self = this;
  ...
  addCustomFilter:function( name, filter ){
      this.filterCustom[name] = filter;
      this._filterData();
  },

  // deregister an existing custom filter
  removeCustomFilter:function( name ){
      delete this.filterCustom[name] ;
      this._filterData();
  },

  // return all active custom filter
  ...
  _filterData:function(){
    ...
    for ( var index in self.filterCustom ) {
        self.activeData = self.activeData.filter( function(row){
          return self.filterCustom[index](row);
        });
    }
  ...
  //iterate through nested sorters
